...
module.exports = function generateSchema (hyperschema, { esm = false, filename } = {}) {
  ...
  function dedupConst (name, def) {
    if (dedup.has(def)) return `const ${name} = ${dedup.get(def)}\n`
    dedup.set(def, name)
    ...
    if (struct.isArray) {
      ...
      for (let i = 0; i < struct.fields.length; i++) {
        ...
        if (structsByName.get(field.type.fqn)?.encoder === null) {
          ...
          d += dedupConst(fieldId, getGeneratedFieldEncoder(field))
          deferred.push(d)
        } else {
          str += `// ${struct.fqn}.${field.name}\n`
          str += dedupConst(fieldId, getGeneratedFieldEncoder(field))
        }
      }
