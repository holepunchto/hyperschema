...
module.exports = function generateSchema (hyperschema, { esm = false } = {}) {
  ...
  for (let i = 0; i < structs.length; i++) {
    ...
    result.encoder = struct.enum ? generateEnum(result.id, struct) : generateStruct(result.id, struct)
  }
  ...
  function generateStruct (id, struct) {
    ...
    if (struct.isArray) {
      ...
      for (let i = 0; i < struct.fields.length; i++) {
        ...
        if (structsByName.get(field.type.fqn)?.encoder === null) {
          deferred += `// ${struct.fqn}.${field.name}, deferred due to recusive use\n`
          deferred += `const ${fieldId} = ${getGeneratedFieldEncoder(field)}\n`
        } else {
          str += `// ${struct.fqn}.${field.name}\n`
          str += `const ${fieldId} = ${getGeneratedFieldEncoder(field)}\n`
        }
      }
    ...
    if (struct.isArray) {
      if (struct.framed) {
        str += `const ${id} = c.array(c.frame(${enc}))\n`
      } else {
        str += `const ${id} = c.array(${enc})\n`
      }
    } else {
      str += `const ${id} = ${enc}\n`
    }
