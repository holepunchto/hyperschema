  ...
  function generateStruct(id, struct, deferred) {
    ...
    function generateEncode({ preencode = false } = {}) {
      ...
      for (let i = 0; i < struct.optionals.length; i++) {
        ...
        flags.push(
          '(' +
            vInlinePrefix(field.version, gen('m', field.name)) +
            ' ? ' +
            field.flag +
            ' : 0)'
        )
    ...
    function generateDecode() {
      ...
      for (let i = 0; i < struct.fields.length; i++) {
        const field = struct.fields[i]
        if (!field.required || field.type.bool || i === struct.flagsPosition)
          break
        seen++
        if (field.external) continue
        pre.set(field, 'r' + i)
      ...
      str += '    const s = {}\n'
      if (struct.expectsVersion) str += '    s.version = version\n'
      ...
      for (let i = 0; i < struct.fields.length; i++) {
        ...
        if (field.external) {
          if (field.useDefault) {
            str += `    s.${prop} = ${field.getDefaultValue()}\n`
          }
          continue
        }

        if (pre.has(field)) {
          str += `    s.${prop} = ${pre.get(field)}\n`
          continue
        }
        ...
        if (field.required) {
          str += `    s.${prop} = ${getFieldEncoder(field)}.decode(state)\n`
          continue
        }

        if (field.useDefault) {
          str += `    s.${prop} = ${flag} ? ${getFieldEncoder(field)}.decode(state) : ${field.getDefaultValue()}\n`
        } else {
          str += `    if (${flag}) struct.${prop} = ${getFieldEncoder(field)}.decode(state)\n`
        }
      }
