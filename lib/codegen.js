...
module.exports = function generateSchema (hyperschema, { esm = false, filename } = {}) {
  ...
  for (let i = 0; i < structs.length; i++) {
    ...
    result.encoder = struct.isEnum
      ...
      : (
          struct.isAlias
            ? generateAlias(result.id, struct)
  ...
  str += 'function getAlias (name) {\n'
  str += '  switch (name) {\n'
  for (let i = 0; i < structs.length; i++) {
    const struct = structs[i]
    if (struct.isAlias) str += `    case ${s(struct.fqn)}: return ${structsByName.get(struct.fqn).id}\n`
  }
  str += '    default: throw new Error(\'Alias not found \' + name)\n'
  str += '  }\n'
  str += '}\n'
  ...
  if (esm) {
    str += 'export { resolveStruct, getStruct, getEnum, getAlias, getEncoding, encode, decode, setVersion, version }\n'
  } else {
    str += 'module.exports = { resolveStruct, getStruct, getEnum, getAlias, getEncoding, encode, decode, setVersion, version }\n'
  }

  return str

  function generateAlias (id, struct) {
    let str = ''
    ...
    function generateEncode ({ preencode = false } = {}) {
      ...
      return `return c.${struct.type.name}.${encode}(state, m)\n`
    }

    function generateDecode () {
      return `return c.${struct.type.name}.decode(state)\n`
    }
  }
