...
module.exports = function generateSchema(
  ...
  function generateStruct(id, struct, deferred, inlinable) {
    ...
    function generateEncode({ preencode = false } = {}) {
      ...
      const generateFlag = (field, parent = 'm', flag = field.flag) => {
        return '(' +
          vInlinePrefix(field.version, gen(parent, field.name)) +
          ' ? ' +
          flag +
          ' : 0)'
      }
      ...
      if (flags.length || inlines.fields.size) {
        ...
        const formatFlags = (flags, op, indent = '') => {
          if (flags.length === 1) {
            return `flags ${op} ${flags[0].slice(1, -1)}\n`
          }
