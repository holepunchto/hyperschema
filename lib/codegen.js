...
module.exports = function generateSchema(hyperschema, { esm = false, filename } = {}) {
  ...
  function generateStruct(id, struct, deferred, inlinable) {
    ...
    const fields = struct.isArray ? [] : struct.getFields()
    const inlines = fields.filter((f) => f.field.inline)
    const flags = visitFlags('m', null, fields)
    ...
    function generateFlags(indent, flag) {
      const first = flag.name === 'm'
      const mut = first ? (flag.inlines ? 'let' : 'const') : ''
      const flags = flag.flags.filter((f) => f.value).map((f) => f.value)

      let str = combineFlags(indent, mut, first ? '=' : '|=', flags)

      for (const next of flag.flags) {
        if (!next.name || !next.flags.length) continue
        str += `${indent}if (${next.name}) {\n`
        str += generateFlags(`${indent}  `, next)
        str += `${indent}}\n`
      }
    ...
    function visitFlags(name, value, fields) {
      const result = { name, value, inlines: false, flags: [] }

      for (const f of fields) {
        if (f.field.external) continue

        if (!f.field.required) {
          result.flags.push({
            name: null,
            value: generateBitFlag(f.field, name, f.flag),
            inlines: false,
            flags: []
          })
        }
        ...
        if (f.fields) {
          result.inlines = true
          result.flags.push(visitFlags(gen(name, f.field.name), null, f.fields))
        }
      }
    ...
    function generateEncode({ preencode = false } = {}) {
      ...
      if (flags.flags.length) {
        if (struct.maxFlag < 128 && preencode) {
          fastFlags = true
        } else if (!inlinable) {
          str += generateFlags('    ', flags)
          str += '\n'
        }
      }
    ...
    function generateDecode() {
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        let shift = ''
        if (field.inline) {
          console.log('inline', field.name, f.shift)
          shift = `, flags >>> ${f.shift}`
        }
