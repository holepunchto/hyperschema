...
module.exports = function generateSchema (hyperschema, opts = {}) {
  const runtime = opts.runtime || 'hyperschema/runtime.js'
  const enums = []
  const structs = []
  const structsByName = new Map()
  const enumsByName = new Map()

  for (let i = 0; i < hyperschema.orderedTypes.length; i++) {
    const { type } = hyperschema.orderedTypes[i]
    if (type.isStruct) {
      structs.push(type)
    } else if (type.isEnum) {
      enums.push(type)
  ...
  for (let i = 0; i < enums.length; i++) {
    const e = enums[i]
    const id = 'enum' + i
    const encoder = generateEnum(id, e)
    enumsByName.set(e.fqn, { encoder, id })
  }
  ...
  for (let i = 0; i < enums.length; i++) {
    const e = enums[i]
    const { encoder } = enumsByName.get(e.fqn)
    str += encoder
    str += '\n'
  }
  ...
  str += 'const EnumMap = new Map([\n'
  for (let i = 0; i < enums.length; i++) {
    const e = enums[i]
    str += `  ['${e.fqn}', ${enumsByName.get(e.fqn).id}]`
    if (i !== enums.length - 1) str += ',\n'
    else str += '\n'
  }
  str += '])\n\n'
  ...
  str += '}\n'
  str += '\n'
  str += 'function resolveEnum (name) {\n'
  str += '  return EnumMap.get(name)\n'
  str += '}\n'
  str += '\n'
  str += 'module.exports = { resolveStruct, resolveEnum, version }\n'

  return str

  function generateEnum (id, e) {
    let str = ''
    str += `// ${e.fqn}\n`
    str += `const ${id} = {\n`
    for (let i = 0; i < e.values.length; i++) {
      str += `  '${e.values[i]}': ${i}`
      if (i !== e.values.length - 1) str += ','
      str += '\n'
    }
    str += '}\n'
    return str
    ...
    function generateEncode (struct, { preencode = false } = {}) {
      ...
      for (let i = 0; i < struct.fields.length; i++) {
        ...
        if (!field.type.bool) {
          ...
          if (i !== struct.fields.length - 1) str += '\n'
        }
      }
      return str
    }
