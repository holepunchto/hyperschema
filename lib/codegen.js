...
module.exports = function generateSchema (hyperschema, { esm = false, filename } = {}) {
  ...
  function generateUnion (id, struct) {
    ...
    for (let i = 0; i < struct.variants.length; i++) {
      ...
      variants.set(variant.type, [variantId, variant.name])
    ...
    function generateEncode ({ preencode = false } = {}) {
      const encode = preencode ? 'preencode' : 'encode'
      let str = ''
      str += `    c.string.${encode}(state, m.type)\n`
      str += '    switch (m.type) {\n'
      for (const [type, [id]] of variants.entries()) {
        str += `      case ${s(type.fqn)}:\n`
        str += `        ${id}.${encode}(state, m.value)\n`
        str += '        break\n'
      }
      str += '      default: throw new Error("Unknown union variant: " + m.type)\n'
      str += '    }\n'
      return str
    }
    ...
    function generateDecode () {
      let str = ''
      str += '    const type = c.string.decode(state)\n'
      str += '    switch (type) {\n'
      for (const [type, [id, name]] of variants.entries()) {
        str += `      case ${s(type.fqn)}:\n`
        str += `        return { name: ${s(name)}, type, value: ${id}.decode(state) }\n`
      }
      str += '      default: throw new Error("Unknown union variant: " + type)\n'
      str += '    }\n'
      return str
    }
  }
