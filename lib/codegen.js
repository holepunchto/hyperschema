...
module.exports = function generateSchema (hyperschema) {
  ...
  for (let i = 0; i < types.length; i++) {
    const { type, fqn } = types[i]
    type.preprocessEncoding()
    encoderMap.set(fqn, 'encoding' + i)
  }

  let str = ''
  str += '// !!! This file is autogenerated by the hyperschema compiler\n' 
  str += `// VERSION ${hyperschema.version}\n`
  str += `const c = require('compact-encoding')\n\n`

  for (let i = 0; i < types.length; i++) {
    const { fqn, type } = types[i]
    str += generateEncoder(encoderMap, encoderMap.get(fqn), fqn, type)  
    str += '\n'
  }
...
function generateEncoder (encoderMap, id, name, type) {
  ...
  for (let i = 0; i < type._encodables.length; i++) {
    ...
    if (enc.framed) typeStr = `c.framed(${typeStr})`
