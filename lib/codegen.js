...
module.exports = function generateSchema (hyperschema, { esm = false, filename } = {}) {
  ...
  for (let i = 0; i < structs.length; i++) {
    ...
    if (struct.isEnum) {
      result.encoder = generateEnum(result.id, struct)
    } else if (struct.isUnion) {
      result.encoder = generateUnion(result.id, struct)
    } else if (struct.isVersioned) {
      result.encoder = generateVersioned(result.id, struct)
    } else {
      result.encoder = generateStruct(result.id, struct, deferred)
    }
  }
  ...
  str += '    default: throw new Error("Union not found: " + name)\n'
  str += '  }\n'
  str += '}\n'
  ...
  if (esm) {
    str += 'export { resolveStruct, getStruct, getEnum, getUnion, getEncoding, encode, decode, setVersion, version }\n'
  } else {
    str += 'module.exports = { resolveStruct, getStruct, getEnum, getUnion, getEncoding, encode, decode, setVersion, version }\n'
  }
  ...
  function generateEnumConstants (id, keys, values = null, offset = 0) {
    let str = `const ${id}_enum = {\n`

    for (let i = 0; i < keys.length; i++) {
      const key = keys[i]
      const value = values ? values[i] : i + offset
      str += `  ${gen.property(key)}: ${value}${i < keys.length - 1 ? ',' : ''}\n`
    }
  ...
  function generateEnum (id, struct) {
    ...
    if (struct.strings) {
      const values = struct.enum.map(e => s(e.key))
      str += generateEnumConstants(id, struct.enum.map(e => e.key), values)
    } else {
      str += generateEnumConstants(id, struct.enum.map(e => e.key), null, struct.offset)
    }
  ...
  function generateUnion (id, struct) {
    const variants = new Map()

    let str = ''
    let enc = ''

    for (let i = 0; i < struct.variants.length; i++) {
      const variant = struct.variants[i]
      const variantId = `${id}_${i}`
      variants.set(variant.name, [variantId, variant.type])

      const variantEnc = getGeneratedTypeEncoder(variant.type)
      str += `// ${struct.fqn}.${variant.name}\n`
      str += dedupConst(variantId, variantEnc)
    }

    str += '\n'

    str += `// ${struct.fqn} variant enum\n`
    str += generateEnumConstants(id, Array.from(variants.keys()))
    ...
    enc += '  },\n'
    enc += `  enum: ${id}_enum,\n`
    enc += `  variants: [${Array.from(variants.keys()).map(name => s(name)).join(', ')}]\n`
    enc += '}'
    ...
    function generateEncode ({ preencode = false } = {}) {
      const encode = preencode ? 'preencode' : 'encode'
      let str = ''
      str += '    if (m == null || typeof m !== "object") throw new Error("Union value must be an object")\n'
      str += '    const keys = Object.keys(m)\n'
      str += '    if (keys.length !== 1) throw new Error("Union value must have exactly one variant key")\n'
      str += '    const variantName = keys[0]\n'
      str += `    const variantIndex = ${id}_enum[variantName]\n`
      str += '    if (variantIndex === undefined) {\n'
      str += `      throw new Error('Unknown union variant "' + variantName + '". Expected one of: ' + Object.keys(${id}_enum).join(', '))\n`
      str += '    }\n'
      str += `    c.uint.${encode}(state, variantIndex)\n`
      str += '    switch (variantIndex) {\n'
      let i = 0
      for (const [, [id]] of variants.entries()) {
        str += `      case ${i}: ${id}.${encode}(state, m[variantName]); break\n`
        i++
      }
      str += '    }\n'
      return str
    ...
    function generateDecode () {
      let str = ''
      str += '    const variantIndex = c.uint.decode(state)\n'
      str += '    switch (variantIndex) {\n'
      let i = 0
      for (const [name, [id]] of variants.entries()) {
        str += `      case ${i}:\n`
        str += `        return { ${gen.property(name)}: ${id}.decode(state) }\n`
        i++
      }
      str += `      default: throw new Error("Unknown union variant index: " + variantIndex + ". Valid indices: 0-${variants.size - 1}")\n`
      str += '    }\n'
      return str
    }
  }
  ...
  function generateStruct (id, struct, deferred) {
    ...
    function generateDecode () {
      ...
      for (let i = 0; i < struct.fields.length; i++) {
        ...
        str += `      ${prop}: ${flag} ? ${getFieldEncoder(field)}.decode(state) : ${field.getDefaultValue()}${end}`
      }
