...
module.exports = function generateSchema(hyperschema, { esm = false, filename } = {}) {
  ...
  function generateVersioned(id, struct, deferred) {
    ...
    function generateDecode() {
      ...
      for (let i = 0; i < struct.versions.length; i++) {
        ...
        str += ' {\n'
        str += `        const decoded = ${getEncoder(def)}.decode(state, v)\n`
  ...
  function generateStruct(id, struct, deferred, inlinable) {
    ...
    function generateFlags(indent, flag, parent = 'm') {
      const first = !flag
      const entries = (flag ? flag.fields : fields).filter(
        (f) => !f.field.external && !f.field.isConstant
      )
    ...
    function generateEncode({ preencode = false } = {}) {
      ...
      for (let i = 0; i < struct.fields.length; i++) {
        ...
        // the flags may be positioned at a constant (or external) field, they are
        // not on the wire themselves but the flags still have to be emitted here
        ...
        if (field.external || field.isConstant) continue
    ...
    function generateDecode() {
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        if (i === struct.flagsPosition) break
        if (field.isConstant) {
          seen++
          continue
        }
        if (!field.required || isFlagField(field) || field.inline) {
          break
        }
        seen++
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        if (field.isConstant) {
          str += `      ${prop}: ${constantLiteral(field.constant)}${end}`
          continue
        }
        ...
        // a bitwise uint lives in the flags too, but as a masked value, not a bool
        if (isFlagField(field) && !field.type.bitwise) {
          str += `      ${prop}: ${flag}${end}`
          continue
        }
...
function constantLiteral(value) {
  return typeof value === 'string' ? s(value) : String(value)
}

function alwaysOptional(struct) {
  for (let i = 0; i < struct.fields.length; i++) {
    const f = struct.fields[i]
    if (f.required || f.isConstant) continue
