...
test(
  'swift: struct with single required uint — roundtrip',
...
test(
  'swift: basic struct, all required fields, no version bump',
  { skip: process.platform === 'win32' },
  (t) => {
...
test(
  'swift: basic struct, all required fields, version bump',
  { skip: process.platform === 'win32' },
  (t) => {
...
test(
  'swift: struct with multiple required uint fields — roundtrip',
  { skip: process.platform === 'win32' },
  (t) => {
    const schema = Hyperschema.from(null)
    schema.namespace('test').register({
